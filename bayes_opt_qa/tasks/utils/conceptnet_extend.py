    ...
    def run(self, data, skip_non_taxonomical=True):
        ...
        for id, fact_info in tqdm(data.items(), "Mapping arguments"):
            ...
            try:
                ...
                logger.warning(f"Unable to parse: {fact}")
                continue
            arg1 = arg1.lstrip().strip()
            arg2 = arg2.lstrip().strip()
            if "." in arg2:
                arg2 = arg2.replace(".", "")
            if len(arg2.split()) > 3:
            ...
            if arg1 == arg2:
                continue
            if fact_info["source"] == "Worldtree":
                accepted_grounding.add(arg2)
                # accepted_grounding.add(arg1)
        ...
        for arg1, extended_data in tqdm(
            ...
            for arg2, connector in extended_data:
                if arg2 in accepted_grounding or True:
