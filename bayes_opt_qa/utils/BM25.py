...
class BM25:
    def fit(self, corpus, ids):
        self.corpus = corpus
        self.ids = ids
        self.joined_corpus = []
        for fact in corpus:
            self.joined_corpus.append(" ".join(fact))
        self.vectorizer = BM25Vectorizer().fit(self.joined_corpus)
    ...
    def query(self, query):
        ...
        TFIDF_dist = cosine_distances(transformed_query, self.transformed_corpus)[0]

        res = []

        for index in np.argsort(TFIDF_dist):
            t_id = self.ids[index]
